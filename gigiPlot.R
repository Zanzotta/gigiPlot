...
gigiPlot <- function(x, y = NULL){
  ...
  switch (type,
          ...
          freqhist = ggplot(dat, aes(x=x, y=after_stat(x))) + 
            ...
            ggtitle(paste("Histogram of ",names(dat)[1] , sep = "")) +
          ...
          scatterplot = ggplot(dat, aes(x=x, y=y)) + 
            ...
            ggtitle(paste("Scatterplot, ",names(dat)[1],"vs",names(dat)[2] , sep = " ")) +
            theme_minimal(base_size = 12),
          
          path = ggplot(dat, aes(x=x, y=y)) + 
            geom_path(linewidth=0.75) + 
            ggtitle(paste("Geom_Path ",names(dat)[1],"with",names(dat)[2] , sep = " ")) +
            theme_minimal(base_size = 12),
          
          boxplot = ifelse(is.factor(y) == T,
                                print(ggplot(dat, aes(x=x, y=y, fill=y)) + 
                                  geom_boxplot() + 
                                  ggtitle(paste("Boxplot of",names(dat)[1], sep = " ")) +
                                  theme_minimal(base_size = 12)),
          ...
          linestep = ggplot(dat, aes(x=x, y=y)) + 
            ...
            ggtitle(paste("Geom_Step ",names(dat)[1],"with",names(dat)[2] , sep = " ")) +
            theme_minimal(base_size = 12),
          
          line = ggplot(dat, aes(x=x, y=y)) + 
            geom_line(linewidth=0.75) + 
            ggtitle(paste("Geom_Line ",names(dat)[1],"with",names(dat)[2] , sep = " ")) +
            theme_minimal(base_size = 12),
          
          bar = ggplot(dat, aes(x=x, y=y)) + 
            geom_bar(stat="identity",fill="blue1", color="white") + 
            ggtitle(paste("Bar Chart ",names(dat)[1],"with",names(dat)[2] , sep = " ")) +
            theme_minimal(base_size = 12),
          
          bar_flip = ifelse(is.factor(x) == T,
                            print(ggplot(dat, aes(x=x, y=after_stat(count))) + 
                                    geom_bar(fill="blue1", color="white") + 
                                    ggtitle(paste("Bar Chart of ",names(dat)[1], sep = " ")) +
                                    coord_flip() +
                                    theme_minimal(base_size = 12)),
